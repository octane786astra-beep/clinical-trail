...
def save_cohort(db: Session, result: dict, criteria: dict, cohort_name: str = None) -> Cohort:
    ...
    def _sanitize(obj):
        """Convert numpy/datetime types to JSON-safe Python primitives."""
    ...
    raw_patients = qr.get("selected_patients", [])
    safe_patients = _sanitize(raw_patients)
    safe_criteria = _sanitize(criteria)
    ...
    cohort = Cohort(
        ...
        total_response_score       = float(qr.get("total_response_score", 0) or 0),
        total_cost                 = float(qr.get("total_cost", 0) or 0),
        qubits_used                = qr.get("qubits_used"),
        shots                      = qr.get("shots"),
        unique_states_explored     = qr.get("unique_states_explored"),
        circuit_depth              = qr.get("circuit_depth"),
        selected_patients          = safe_patients,
